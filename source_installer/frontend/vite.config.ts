...
export default defineConfig(({ mode }) => {
  const common = {
    base: '',
    plugins: [react(),pluginRewriteAll(), eslint(), tsconfigPaths()],
    server: {
      // Proxy HTTP requests to the flask server
      proxy: {
        '/outputs': {
          target: 'http://127.0.0.1:80/outputs',
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/outputs/, ''),
        },
        '/upload': {
          target: 'http://127.0.0.1:80/upload',
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/upload/, ''),
        },
        '/flaskwebgui-keep-server-alive': {
          target: 'http://127.0.0.1:80/flaskwebgui-keep-server-alive',
          changeOrigin: true,
          rewrite: (path) =>
