...
import { terminateRequest, terminateRequestWithEmail } from 'app/actions';
import {
  terminateRequestGen,
  terminateOneRequest,
  prepareNextRequest,
  ...
  return {
    ...
    onGenerationResult: (data: InvokeAI.ImageResultResponse) => {
      try {
        ...
        const { requested, requests } = state.generationReducer;
        // const { requested, id, tic, status } = state.generationReducer;
        const { boundingBox: _, generationMode, ...rest } = data;
        console.log('data_result', data);
        const waitingTime = new Date().getTime() - requests[0].tic;
        console.log('*******requested, requests', requested, requests);
        if (requested == 1) {
          if (requests[0].status == 1) {
            console.log('id', requests[0].id);
            terminateRequest(requests[0].id, waitingTime, data.url)
              .then((res) => {                
                if (requests.length > 1) {
                  console.log(']]]]]]]]]]]]]]', res)
                  dispatch(prepareNextRequest());                  
                }
                dispatch(terminateOneRequest(requests[0].id));
                // dispatch(terminateRequestGen());
              })
              .catch((err) => {
                console.log(err);
              });
          }
          if (requests[0].status == 3) {
            console.log('id', requests[0].id);
            terminateRequestWithEmail(requests[0].id, waitingTime, data.url)
              .then((res) => {
                if (requests.length > 1) {
                  dispatch(prepareNextRequest());                  
                }
        ...
        if (['txt2img', 'img2img', 'unifiedCanvas'].includes(generationMode)) {
          dispatch(
            addImage({
              category: 'result',
